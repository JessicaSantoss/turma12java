...
public class Security extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		// permite o cadastro de o usuario
		http.csrf().disable().authorizeRequests().antMatchers(HttpMethod.GET, "/").permitAll()
				.antMatchers(HttpMethod.POST, "/cadastrar").permitAll()

				// concede as permissões para o usuario ADMIN para que ele possa usar os ends
				// points
	...
	// entra na excessão de que se não for o usuario ADMIN, ele não deixa ir para os
	// ends points
	@Override
